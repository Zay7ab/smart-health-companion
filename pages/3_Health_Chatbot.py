...
/* Vitals Cards (Matches your Screenshot exactly) */
...
/* Sidebar Inputs Fix */
div[data-testid="stSidebar"] { background-color: #050805 !important; }
label { color: #4ade80 !important; font-weight: 600; }
</style>
...
# --- Sidebar (Now Controlling Values) ---
with st.sidebar:
    st.markdown("### ⚙️ Live Vitals Control")
    # Widgets directly update session state via 'key'
    st.text_input("Blood Pressure", key="bp")
    st.number_input("Heart Rate (BPM)", key="hr")
    st.number_input("Body Temp (°F)", key="temp", format="%.1f")
    st.number_input("Oxygen Sat (%)", key="ox")
...
<div class="topbar">
    <div>
        ...
        <div style="font-size: 12px; color: rgba(255,255,255,0.4);">Diagnostic Interface Active</div>
    </div>
    <div class="ai-badge"><span class="ai-dot"></span> FastAPI + Groq AI Active</div>
</div>
""", unsafe_allow_html=True)

# --- Vitals Display (Reading directly from Session State) ---
st.markdown(f"""
...
with tab_chat:
    ...
    with display_chat:
        st.markdown('<div class="bubble bubble-ai"><div style="font-size:11px; font-weight:700; color:#4ade80; margin-bottom:8px; text-transform:uppercase;">ClinIQ Intelligence</div>System ready. Vitals are synced. How can I help?</div>', unsafe_allow_html=True)
