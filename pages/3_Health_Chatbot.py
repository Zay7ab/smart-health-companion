...
# --- Full Dark Neon CSS (Matching Heart Disease Page) ---
st.markdown("""
<style>
...
for i, meta in enumerate(v_meta):
    with v_cols[i]:
        ...
        with st.popover(f"Edit {meta['key'].upper()}"):
            ...
            if st.button(f"Update {meta['key']}"): st.rerun()
    ...
    if query:
        ...
        with st.spinner("🤖 Analyzing clinical context..."):
            ...
            try:
                ...
                reply = "⏱️ API Timeout. Please check connection."
...
with tab_tools:
    ...
    with c2:
        if st.button("📝 Prepare Clinical Report"):
            ...
            st.download_button("💾 Download PDF Summary", data=pdf_data, file_name="ClinIQ_Report.pdf", mime="application/pdf")
