...
.vital-card-container {
    ...
    text-align: center; border-top: 2px solid #4ade80;
}
.vital-label { 
...
.bubble-user { 
    background: #0f1a0f; border: 1px solid #1a2e1a; 
    margin-left: auto; border-right: 4px solid #58a6ff; 
    color: #ffffff; 
}
</style>
        ...
        with st.popover(f"Edit {meta['key'].upper()}"):
            if meta["key"] == "bp":
                new_val = st.text_input(
                    f"New {meta['label']}",
                    value=str(st.session_state[meta["key"]])
                )
            else:
                new_val = st.number_input(
                    f"New {meta['label']}",
                    value=float(st.session_state[meta["key"]])
                )
            ...
            try:
                res = requests.post(
                    ...
                    json={
                        "message": context + query, 
                        "history": st.session_state.chat_history[:-1],
                        "api_key": st.secrets.get("GROQ_API_KEY", "your_fallback_key")
                    }, 
                    timeout=20
                )
                res.raise_for_status()
