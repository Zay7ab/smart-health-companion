...
.emergency-hero { background: linear-gradient(135deg,#1a0000,#2d0000); border: 1px solid #3d0000; border-radius: 20px; padding: 2rem 2.5rem; margin-bottom: 1.5rem; }
.emergency-hero h1 { font-size: 2rem; font-weight: 800; color: #ffffff; margin-bottom: 0.5rem; }
.emergency-hero p { font-size: 13px; color: rgba(255,255,255,0.5); line-height: 1.6; max-width: 600px; }
.emergency-badge { display: inline-flex; align-items: center; gap: 6px; background: rgba(239,68,68,0.15); border: 1px solid rgba(239,68,68,0.3); border-radius: 20px; padding: 4px 12px; font-size: 11px; color: #ef4444; font-weight: 600; letter-spacing: 1px; margin-bottom: 1rem; }
.country-card { background: #0d120d; border: 1px solid #1a2e1a; border-radius: 12px; padding: 1rem; margin-bottom: 8px; display: flex; align-items: center; gap: 12px; }
.country-flag { font-size: 1.5rem; flex-shrink: 0; }
.country-name { font-size: 13px; font-weight: 700; color: #ffffff; margin-bottom: 2px; }
.country-numbers { font-size: 12px; color: rgba(255,255,255,0.5); }
.warning-signs { display: grid; grid-template-columns: repeat(2,1fr); gap: 10px; margin-bottom: 1.5rem; }
.warning-card { background: #0d120d; border: 1px solid rgba(239,68,68,0.2); border-radius: 12px; padding: 1rem; display: flex; align-items: center; gap: 10px; }
.warning-icon { font-size: 1.5rem; flex-shrink: 0; }
.warning-text { font-size: 13px; font-weight: 600; color: #ef4444; }
.warning-sub { font-size: 11px; color: rgba(255,255,255,0.4); margin-top: 2px; }
.form-card { background: #0d120d; border: 1px solid #1a2e1a; border-radius: 16px; overflow: hidden; margin-bottom: 1rem; }
.form-header { padding: 1rem 1.25rem; background: rgba(239,68,68,0.05); border-bottom: 1px solid rgba(239,68,68,0.15); display: flex; align-items: center; justify-content: space-between; }
.form-header h2 { font-size: 14px; font-weight: 600; color: #ef4444; }
.form-tag { font-size: 10px; color: #ef4444; background: rgba(239,68,68,0.1); padding: 2px 8px; border-radius: 20px; font-weight: 600; }
.form-body { padding: 1.25rem; }
.ai-insight { background: #0d120d; border: 1px solid rgba(239,68,68,0.2); border-radius: 12px; padding: 1rem; margin-top: 1rem; }
.ai-insight-header { font-size: 11px; font-weight: 600; color: #ef4444; text-transform: uppercase; letter-spacing: 0.5px; margin-bottom: 8px; }
.ai-insight-text { font-size: 13px; color: rgba(255,255,255,0.7); line-height: 1.7; }
.steps-card { background: linear-gradient(135deg,#1a0000,#2d0000); border: 1px solid rgba(239,68,68,0.2); border-radius: 16px; padding: 1.5rem; margin-bottom: 1rem; }
.step-item { display: flex; align-items: flex-start; gap: 10px; margin-bottom: 10px; }
.step-num { width: 24px; height: 24px; border-radius: 50%; background: rgba(239,68,68,0.2); display: flex; align-items: center; justify-content: center; font-size: 11px; font-weight: 700; color: #ef4444; flex-shrink: 0; }
.step-text { font-size: 12px; color: rgba(255,255,255,0.7); line-height: 1.5; }
...
div[data-testid="stButton"] button { background: linear-gradient(135deg,#7f0000,#ef4444) !important; color: white !important; border: none !important; border-radius: 8px !important; font-weight: 700 !important; font-size: 13px !important; padding: 0.6rem 1.5rem !important; }
...
tab1, tab2, tab3, tab4, tab5, tab6 = st.tabs(["🌍 Middle East", "🌍 Europe", "🌎 Americas", "🌏 Asia Pacific", "🌍 Africa", "🌐 Universal"])

with tab1:
    countries = [
        ("🇦🇪","UAE","Police: 999 | Ambulance: 998 | Fire: 997"),
        ("🇸🇦","Saudi Arabia","Police: 999 | Ambulance: 911 | Fire: 998"),
        ("🇶🇦","Qatar","All: 999"),("🇰🇼","Kuwait","All: 112"),
        ("🇧🇭","Bahrain","All: 999"),("🇴🇲","Oman","All: 9999"),
        ("🇯🇴","Jordan","All: 911"),("🇱🇧","Lebanon","Police: 112 | Ambulance: 140"),
    ...
    for flag, country, numbers in countries:
        st.markdown(f'<div class="country-card"><div class="country-flag">{flag}</div><div><div class="country-name">{country}</div><div class="country-numbers">{numbers}</div></div></div>', unsafe_allow_html=True)
...
with tab2:
    countries = [
        ...
        ("🇳🇱","Netherlands","All: 112"),("🇧🇪","Belgium","Police: 101 | Ambulance: 100"),
        ("🇨🇭","Switzerland","Police: 117 | Ambulance: 144 | Fire: 118"),
        ("🇸🇪","Sweden","All: 112"),("🇵🇱","Poland","Police: 997 | Ambulance: 999 | Fire: 998"),
...
with tab3:
    countries = [
        ("🇺🇸","United States","All: 911"),("🇨🇦","Canada","All: 911"),
        ("🇲🇽","Mexico","All: 911"),
...
with tab4:
    countries = [
        ...
        ("🇦🇺","Australia","All: 000"),("🇳🇿","New Zealand","All: 111"),
...
with tab6:
    st.markdown("""
    <div class="country-card" style="border-color:rgba(239,68,68,0.3);">
        <div class="country-flag">🌐</div>
        <div><div class="country-name">Universal Emergency Number</div>
        <div class="country-numbers"><b style="color:#4ade80;">112</b> — Works in most countries worldwide</div></div>
    </div>
    <div class="country-card">
        <div class="country-flag">📱</div>
        <div><div class="country-name">Mobile Emergency</div>
        <div class="country-numbers">112 works even without SIM card on most phones</div></div>
    </div>
    """, unsafe_allow_html=True)
...
with col1:
    emergency_type = st.selectbox("Emergency Type", ["Heart Attack","Stroke","Choking","Severe Bleeding","Burns","Fracture / Broken Bone","Allergic Reaction","Seizure / Epilepsy","Drowning","Electric Shock","Poisoning / Overdose","Diabetic Emergency","Asthma Attack","Head Injury","Loss of Consciousness","Snake / Animal Bite","Heat Stroke","Hypothermia","Panic Attack","Other Emergency"])
...
if st.button("🚨 GET IMMEDIATE FIRST AID INSTRUCTIONS"):
    with st.spinner("🤖 FastAPI getting emergency instructions..."):
        try:
            response = requests.post(f"{API_URL}/emergency/firstaid", json={"emergency_type": emergency_type, "country": country, "patient_age": patient_age, "conscious": conscious, "situation": situation, "api_key": st.secrets.get("GROQ_API_KEY", "")}, timeout=30)
            result = response.json()
            if "error" in result:
                st.error(f"API Error: {result['error']}")
            else:
                st.markdown(f'<div class="ai-insight"><div class="ai-insight-header">🚨 Emergency First Aid — {emergency_type}</div><div class="ai-insight-text">{result["instructions"].replace(chr(10), "<br>")}</div></div>', unsafe_allow_html=True)
...
with col1:
    ...
    <div class="steps-card">
        <div style="font-size:13px;font-weight:700;color:#ef4444;margin-bottom:1rem;text-transform:uppercase;letter-spacing:1px;">🫀 CPR Steps (Adult)</div>
...
with col2:
    st.markdown("""
    <div class="steps-card">
        <div style="font-size:13px;font-weight:700;color:#ef4444;margin-bottom:1rem;text-transform:uppercase;letter-spacing:1px;">🧠 FAST Stroke Test</div>
...
st.markdown('<div class="disclaimer">⚠️ For educational purposes only. Always call emergency services immediately in life-threatening situations.</div>', unsafe_allow_html=True)
