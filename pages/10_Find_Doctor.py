...
with col2:
    specialty = st.selectbox("Medical Specialty", [
        "Auto-detect from condition", "Cardiologist", "Pulmonologist",
        "Neurologist", "Oncologist", "Orthopedic Surgeon",
        "Gastroenterologist", "Endocrinologist", "General Physician",
            ...
            try:
                ...
                if "error" in result:
                    ...
                    <div class="ai-insight">
                        <div class="ai-insight-header">🤖 AI Doctor Recommendations (via FastAPI) — {condition} in {city}</div>
                        <div class="ai-insight-text">{result['recommendations'].replace(chr(10), '<br>')}</div>
                    </div>
                    """, unsafe_allow_html=True)

                    st.markdown(f"""
                    <div style="background:linear-gradient(135deg,#1a3a1a,#2d5a1a);border-radius:16px;padding:1.5rem;margin-top:1rem;">
                        <div style="font-size:11px;color:#97c459;font-weight:600;text-transform:uppercase;letter-spacing:1px;margin-bottom:10px;">⚡ Quick Action Steps</div>
                        <div style="display:grid;grid-template-columns:repeat(3,1fr);gap:10px;">
                            <div style="background:rgba(255,255,255,0.1);border-radius:10px;padding:0.75rem;text-align:center;">
                                ...
                                <div style="font-size:10px;color:rgba(255,255,255,0.7);">Book before visiting</div>
                            </div>
                            <div style="background:rgba(255,255,255,0.1);border-radius:10px;padding:0.75rem;text-align:center;">
                                <div style="font-size:1.5rem;margin-bottom:4px;">📋</div>
                                <div style="font-size:11px;color:white;font-weight:600;">Bring Records</div>
                                <div style="font-size:10px;color:rgba(255,255,255,0.7);">Previous tests & meds</div>
                            </div>
                            <div style="background:rgba(255,255,255,0.1);border-radius:10px;padding:0.75rem;text-align:center;">
                                <div style="font-size:1.5rem;margin-bottom:4px;">⭐</div>
                                <div style="font-size:11px;color:white;font-weight:600;">Check Reviews</div>
                                <div style="font-size:10px;color:rgba(255,255,255,0.7);">Verify credentials</div>
                            </div>
                        </div>
                    </div>
                    """, unsafe_allow_html=True)
